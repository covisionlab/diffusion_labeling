...
class Transformer(nn.Module):
    def __init__(self, n_ctx: int, width: int, layers: int, heads: int):
        ...
        self.resblocks = nn.ModuleList(
            [
                ResidualAttentionBlock(n_ctx, width, heads)
...
class LayoutTransformerEncoder(nn.Module):
    def __init__(
        ...
        if not self.not_use_layout_fusion_module:
            self.transform = Transformer(n_ctx=layout_length, width=hidden_dim, layers=num_layers, heads=num_heads)
        ...
        for resolution in self.resolution_to_attention:
            resolution_y, resolution_x = resolution
            interval_y = 1.0 / resolution_y
            interval_x = 1.0 / resolution_x
            self.image_patch_bbox_embedding['resolution{}'.format(resolution)] = torch.FloatTensor(
                [(interval_y * j, interval_x * i, interval_y * (j + 1), interval_x * (i + 1)) for i in range(resolution_x) for j in range(resolution_y)],
            ).cuda()  # (L, 4)
